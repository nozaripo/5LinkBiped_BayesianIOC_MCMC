function [results,chain,s2chain,sschain, hchain]=mcmcrun1(model,data,params,options,res,k)
...
priortype= getpar(model,'priortype',[]);  % prior type, 1 = Gaussian; 
...
if dodram
  RDR = getpar(options,'RDR',[]); % RDR qiven in options
  if ~isempty(RDR)
    for i=1:Ntry
...
% oldprior = feval(priorfun,oldpar);
...
%     newprior = feval(priorfun,newpar);
