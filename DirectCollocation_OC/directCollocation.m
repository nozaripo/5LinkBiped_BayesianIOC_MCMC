...
guess.state = [G.state(:,1), repmat(B.state.low,1,nGrid-2)+repmat(B.state.upp-B.state.low,1,nGrid-2).*rand(size(B.state.low,1),nGrid-2) , G.state(:,end)];

guess.control = interp1(G.time', G.control', guess.time')';
guess.control = [repmat(B.control.low,1,nGrid)+repmat(B.control.upp-B.control.low,1,nGrid).*rand(1,nGrid)];
...
% xLow = [[G.state(1:5,1);-inf*ones(5,1)], B.state.low*ones(1,nGrid-2), B.finalState.low]; %
...
% xUpp = [[G.state(1:5,1);+inf*ones(5,1)], B.state.low*ones(1,nGrid-2), B.finalState.upp]; %
...
n_res = 100;
tt = linspace(0,t(end),n_res);
xx = interp1(t,x',tt,'spline')';
uu = interp1(t,u',tt,'spline')';
...
%     integrand = pathObj(t,x,u);  %Calculate the integrand of the cost function
    weights = ones(n_res,1);
    weights([1,end]) = 0.5;
    integrand = pathObj(tt,xx,uu);  %Calculate the integrand of the cost function
