...
%mkdir strcat('/scratch1/nozaripo/LocalWorkDir/', getenv('SLURM_JOB_ID')) strcat(getenv('SLURM_ARRAY_TASK_ID'))
%pc.JobStorageLocation = strcat('/scratch1/nozaripo/LocalParpoolDir_MCMC/', getenv('SLURM_JOB_ID'), '/', getenv('SLURM_ARRAY_TASK_ID'))
% pc.JobStorageLocation = strcat('/scratch1/nozaripo/LocalParpoolDir_MCMC/', getenv('SLURM_JOB_ID'))
...
% poolobj = parpool(pc, n_pools * length(V_Treadmill_Array));
poolobj = parpool(pc, n_pools*(length(V_Treadmill_Array)-2));
...
% Stop the clock
runtime = toc/60;
disp(['el_time:   ', num2str(runtime), ' mins'])

%cd(foldername)
% filename = ['Results_',id_file,'.mat'];
% save(['Results/' foldername, '/', filename, '_', num2str(options.nsimu), 'iter'],'chain', 'results');
filename = ['Results_' datestr(datetime,'yy-mm-dd_HH-MM') '.mat'];
save(filename,'chain', 'results');
...
function ss = MSD_SS(W,data, poolobj)
...
V_Treadmill_Array = data.V_Treadmill_Array;
W1 = W;
% % W1 = 1./(1 + exp(-W));
...
parfor i_v=3:length(V_Treadmill_Array)% comment when you would want to run a single optim
% for i_v=1:length(V_Treadmill_Array)% comment when you would want to run a single optim
    [ xInt(:,:,i_v), tInt(:,:,i_v) ]   =  ForwardSimulation_OneSpeed_simple(W1, V_Treadmill_Array(i_v));
%     ss = ss + sum(rms(xInt(:,:,i_v) - data.xInt(:,:,i_v),2)) + 10*(norm( tInt(1,end,i_v) - data.tInt(1,end,i_v) ));
end
    
    

% ss = rms(rms(xInt-data.xInt,2));
% ss = sum(sum(rms(xInt - data.xInt,2))) + 10*(norm( tInt(1,end,:) - data.tInt(1,end,:) ));
ss = sum(sum(rms(xInt(:,:,3:6) - data.xInt(:,:,3:6),2))) + 10*(norm( squeeze(tInt(1,end,3:6)) - squeeze(data.tInt(1,end,3:6) )));
