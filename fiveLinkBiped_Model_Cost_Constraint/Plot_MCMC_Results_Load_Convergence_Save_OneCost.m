...
ref_data = load('Synthetic_5Weights_5Speeds_10Subjects.mat');
ref_data = load('Synthetic_TorqSqrd_5Speeds_1Subject.mat');
ref_data = load('Synthetic_TorqAbs_7Speeds_1Subject.mat');
ref_data.Weights = zeros(1,16);
ref_data.Weights(7) = 1;
...
iter = 1500;
Num_Sampl_Convergence = iter/2;
foldername = 'Results/2023-03-13';

foldername = 'Results/2023-03-23_OneCost';
sig = .4;
ub = 10;

% For these, I did a logit transformation on the cost weights
iter = 2500;
foldername = 'Results/2023-03-25_OneCost';
sig = .2;
ub = 10;

% The constraints are different in the forward simulations here
iter = 2000;
foldername = 'Results/2023-05-15_OneCost';
sig = .2;
ub = 10;
...
x_density_Select    = [];
KS_Stat_Gaus_Select = [];
Median_Dist_Gaus_Select=[];
KS_Stat_Individual_Select = [];
Median_Dist_Individual_Select= [];
Z_Stat_Individual_Select = [];
...
%% Perform storing data; Convergence Analysis; KS-Stat Evaluation
for id_method=1:2
    if id_method == 1
        FileName_Ext='';
        FolderName_Ext=strcat('_sig',num2str(sig),'_ub', num2str(ub));
        Transform_Matrix = eye(16);
    else
        FileName_Ext = '_Bases';
        FolderName_Ext = strcat('_Bases__sig',num2str(sig),'_ub', num2str(ub));
        Transform_Matrix = pinv(sPCR_loadings);
%         Transform_Matrix = sPCR_loadings';
    end
    for id_Subj = 1:Num_Subjects
        for id_W= 1:Num_Weights
            disp(['Subject : ' num2str(id_Subj)  '    ||    Weight Set : ' num2str(id_W)])
            ...
            for ii=1:3
                filename = strcat('Results_W', num2str(id_W),'_S', num2str(id_Subj),'_Rep', num2str(ii),'_', num2str(iter), 'iter', FileName_Ext, '.mat');
                fullpath = strcat(foldername,FolderName_Ext,'/',filename);
                if exist(fullpath) 
                    data = load(fullpath);
                    results = data.results;
                    max_len = length(results.sss2);
                    
                    chain = data.chain(1:max_len, :) * Transform_Matrix;
                    chain_= chain./sum(chain,2)*sum(ref_data.Weights(id_W,:));
                    chain_= chain./sqrt(sum(chain.^2,2));
%                     chain_= chain./chain(:,7);
                    theta(:,:,ii)  = chain_(end-Num_Sampl_Convergence+1:end, :);
                    
                    % rank normalization
                    rank(:,:,ii) = floor(tiedrank(theta(:,:,ii)));
                    zrank(:,:,ii)= norminv( (rank(:,:,ii)-3/8) / (Num_Sampl_Convergence+1/4) ) ;
                    theta = zrank;
                    
                    % Mixing chains for visualization
                    chain_mixed = [chain_mixed; chain_];
                    acc_ch = [acc_ch; results.accechain(1:max_len,1)];
                    log_pos = [log_pos; results.sss2];
                end
            end

            posterior_threshold = .1;
            indices = find(acc_ch == 1 & log_pos <= posterior_threshold);
            
            while length(indices) < 50
                %                 indices = find(results.accechain == 1 & results.sss2 <= posterior_threshold);
                posterior_threshold = posterior_threshold + .1;
                indices = find(acc_ch == 1 & log_pos <= posterior_threshold);
            end
%             indices = find(acc_ch == 1);
            indices = indices(indices>=Num_Sampl_Convergence);
            
            chain_tmp = chain_mixed(indices,:);

            
            chain_Select = [ chain_Select; reshape(chain_tmp,length(indices)*Num_Costs,1) ];
            Cost_ID_Select = [ Cost_ID_Select; reshape( repmat(1:Num_Costs,length(indices),1), length(indices)*Num_Costs,1) ] ;
            True_Weights_Select = [True_Weights_Select; reshape( repmat(ref_data.Weights(id_W,:),length(indices),1),  length(indices)*Num_Costs,1)];
            Weight_ID_Select = [Weight_ID_Select; id_W*ones(length(indices)*Num_Costs,1) ];
            Subject_ID_Select = [Subject_ID_Select; id_Subj*ones(length(indices)*Num_Costs,1) ];
            Inference_ID_Select = [Inference_ID_Select; id_method*ones(length(indices)*Num_Costs,1) ];
            
            for id_C = 1:Num_Costs
                [h,p,kstat(id_C)] = kstest2(ref_data.Weights(id_W, id_C), chain_tmp(:, id_C) );
                Median_Dist(id_C) = norm( median(chain_tmp(:, id_C)-ref_data.Weights(id_W, id_C) ) );

                
                [y,x]=density(chain_tmp(:, id_C),[], 2);
                xx = linspace(min(x), max(x), size(chain_tmp,1));
%                 yy = interp1(x, y, xx);
                x_density = xx';
                x_density_Select = [x_density_Select; x_density];
                Median_Dist_Gaussian(id_C) = norm( median(x_density-ref_data.Weights(id_W, id_C) ) );
                
%                 [h,p,ci,zval] = ztest(x_density-ref_data.Weights(id_W, id_C), 0, std(x_density));
                zstat(id_C) = abs ( mean(x_density-ref_data.Weights(id_W, id_C)) / std(x_density) ) ;
                
                [h,p,kstat_Gaussian(id_C)] = kstest2(ref_data.Weights(id_W, id_C), x_density );
                KS_Stat_Individual_Select = [ KS_Stat_Individual_Select; kstat(id_C) * ones(length(indices),1)];
                Z_Stat_Individual_Select = [ Z_Stat_Individual_Select; zstat(id_C) * ones(length(indices),1)];
                Median_Dist_Individual_Select = [ Median_Dist_Select; Median_Dist(id_C) * ones(length(indices)*Num_Costs,1)];
...
%                 disp(['W_' num2str(ik) ': Median Distance=' num2str( Median_Dist(ik) , '%.2f') ])
            ...
            KS_Stat_Select = [ KS_Stat_Select; rms(kstat) * ones(length(indices)*Num_Costs,1)];
            Median_Dist_Select = [ Median_Dist_Select; rms(Median_Dist) * ones(length(indices)*Num_Costs,1)];
            KS_Stat_Gaus_Select = [ KS_Stat_Gaus_Select; rms(kstat_Gaussian) * ones(length(indices)*Num_Costs,1)];
            Median_Dist_Gaus_Select = [ Median_Dist_Gaus_Select; rms(Median_Dist) * ones(length(indices)*Num_Costs,1)];

            %% Convergence Analysis for Multiple Variables (Brooks & Gelman, 1997)
%             n = size(theta, 1); % max number of iterations
%             m = size(theta, 3); % number of independent chains
%             theta_jbar = mean(theta, 1);
%             theta_bar  = mean(theta_jbar,3);
%             
%             W_Sum = zeros(Num_Costs,Num_Costs);
%             Bn_Sum= zeros(Num_Costs,Num_Costs);
%             for j=1:m
%                 for t=1:n
%                     W_Sum = W_Sum  + ( theta(t,:,j) - theta_jbar(:,:,j) )' * ( theta(t,:,j) - theta_jbar(:,:,j) ) ;
%                 end
%                 Bn_Sum= Bn_Sum + ( theta_jbar(:,:,j) - theta_bar )' * ( theta_jbar(:,:,j) - theta_bar ) ;
%             end
%             W  = W_Sum  / ( m*(n-1) ) ;
%             Bn = Bn_Sum / (m-1) ;
%             
%             eig ( inv(W)*Bn )
            ...
            %% Convergence analysis for each variable
            n = size(theta, 1); % max number of iterations
            m = size(theta, 3); % number of independent chains
            theta_jbar = mean(theta, 1);
            theta_bar  = mean(theta_jbar,3);
            ...
            for i=1:Num_Costs
                for j=1:m
                    for t=1:n
                        W_Sum = W_Sum  + ( theta(t,i,j) - theta_jbar(:,i,j) )' * ( theta(t,i,j) - theta_jbar(:,i,j) ) ;
                    end
                    Bn_Sum= Bn_Sum + ( theta_jbar(:,i,j) - theta_bar(i) )' * ( theta_jbar(:,i,j) - theta_bar(i) ) ;
                end
            end
            W  = W_Sum  / ( m*(n-1) ) ;
            Bn = Bn_Sum / (m-1) ;
            
            V_hat = (1-1/n)*W + (1+1/m)*Bn;
            R_hat = V_hat / W;
            if Rhat_max<R_hat
                Rhat_max = R_hat;
            end
            if R_hat > 1.1
                Rhat_viol_Subject = [Rhat_viol_Subject; id_Subj];
                Rhat_viol_Weight = [Rhat_viol_Weight; id_W];
            end
...
save('Results_BayesianIOC_2023-05-15_OneCost', 'chain_Select','Cost_ID_Select','True_Weights_Select','Weight_ID_Select','Subject_ID_Select','Inference_ID_Select', 'KS_Stat_Select', 'Median_Dist_Select', 'x_density_Select', 'KS_Stat_Gaus_Select', 'Median_Dist_Gaus_Select', 'KS_Stat_Individual_Select', 'Median_Dist_Individual_Select', 'Z_Stat_Individual_Select')


save('Results_BayesianIOC_2023-03-25_OneCost', 'chain_Select','Cost_ID_Select','True_Weights_Select','Weight_ID_Select','Subject_ID_Select','Inference_ID_Select', 'KS_Stat_Select', 'Median_Dist_Select', 'x_density_Select', 'KS_Stat_Gaus_Select', 'Median_Dist_Gaus_Select', 'KS_Stat_Individual_Select', 'Median_Dist_Individual_Select', 'Z_Stat_Individual_Select')

save('Results_BayesianIOC_2023-03-13', 'chain_Select','Cost_ID_Select','True_Weights_Select','Weight_ID_Select','Subject_ID_Select','Inference_ID_Select', 'KS_Stat_Select', 'Median_Dist_Select', 'x_density_Select', 'KS_Stat_Gaus_Select', 'Median_Dist_Gaus_Select', 'KS_Stat_Individual_Select', 'Median_Dist_Individual_Select')
