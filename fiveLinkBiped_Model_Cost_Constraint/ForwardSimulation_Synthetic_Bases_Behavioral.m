...
Demographics = readtable('../data/WBDSinfo.xlsx');

load('Fukuchi_Angles_GRF_Trajectories.mat')
q_demo = Angles_Data{2,1,2,1};
qi_demo = q_demo(:,1)
qf_demo = q_demo(:,end)
Subject_ID = 1;
...
for i_v=1:length(V_Treadmill_Array)% comment when you would want to run a single optim
    ...
    height = Demographics.Height(Demographics.Subject==Subject_ID);  height = height(1)/100;
    mass = Demographics.Mass(Demographics.Subject==Subject_ID);  mass = mass(1);
...
%     param.I5 = 0.93;  %kg-m^2
    ...
    param.qi_demo = qi_demo;
    param.qf_demo = qf_demo;
    ...
    qi_demo = Angles_Data{Subject_ID, i_v, 2, 1}(:,1);
    qf_demo = Angles_Data{Subject_ID, i_v, 2, 1}(:,end);
%     q0 = qi_demo;
    q0 = qi_demo(:,1);
    tEnd = Time_Data{Subject_ID, i_v, 2, 1}(end);
...
%         problem.func.bndCst = @(t0,x0,tF,xF)( stepConstraint(x0,xF,param) );
        problem.func.bndCst = @(t0,x0,tF,xF)( stepConstraint_behavioral(x0,xF,param,q0) );
        
        problem.func.pathCst = @(t,x,u)( pathConstraint_behavioral(t,x,u,q0,param, V_treadmill) );
        ...
        problem.bounds.finalTime.low = tEnd-.05;
        problem.bounds.finalTime.upp = tEnd+.05;
        ...
        problem.bounds.initialState.low = [qi_demo-.1; dqLow];
        problem.bounds.initialState.upp = [qi_demo+.1; dqUpp];
        ...
        problem.bounds.finalState.low = [qf_demo-.1; dqLow];
        problem.bounds.finalState.upp = [qf_demo+.1; dqUpp];
...
save('Synthetic_5Weights_5Speeds_10Subjects_2023-04-13.mat', 'soln_map', 'V_Treadmill_Array', 'Weights', 'Mass', 'Height')
