...
tbl = data.frame(chain, Cost, True_Weights, Weight, Subject, Inference, KS_Stat, Median_Dist, KS_Stat_Gaus, Median_Dist_Gaus, x_density, KS_Stat_Indiv, Median_Dist_Indiv) %>%
  mutate(Cost = factor(Cost, levels=1:16, labels=Criteria.Names),
         Inference = factor(Inference, labels=c("Naïve Inference", "Featured-Reduced")))
...
Left.Knee = Femur.Stance - Tibia.Stance
Right.Knee = Femur.Swing - Tibia.Swing
Left.Hip = Femur.Stance
Right.Hip = Femur.Swing
...
df <- data.frame(Time, Left.Hip, Right.Hip, Left.Knee, Right.Knee, Speed=Speed, Cost=Cost)
df_Long <- df %>% pivot_longer(cols=2:5, names_to = "Variable", values_to="Joint_Angle")
#View(df_Long)

for(i in 1:length(Cost)){
  if(df_Long$Variable[i]=="Left.Knee"){
    df_Long$Joint[i] = "Knee"
    df_Long$Side[i] = "Left"
  }else if(df_Long$Variable[i]=="Right.Knee"){
    df_Long$Joint[i] = "Knee"
    df_Long$Side[i] = "Right"
  }else if(df_Long$Variable[i]=="Left.Hip"){
    df_Long$Joint[i] = "Hip"
    df_Long$Side[i] = "Left"
  }else{
    df_Long$Joint[i] = "Hip"
...
```{r}
ggplot(df_Long, aes(x=Time, y=Joint_Angle, group=Speed, col=Speed)) +
  geom_line(size=.9) +
  facet_grid(Joint~Side) 
...
df <- data.frame(Time, Left.Hip, Right.Hip, Left.Knee, Right.Knee, Speed=Speed, Cost=Cost, Simulation=Simulation, Obs=Obs, RMS=Error)
df_Traj <- df %>% pivot_longer(cols=2:5, names_to = "Variable", values_to="Joint_Angle")
View(df_Traj)

for(i in 1:length(Error)){
  if(df_Traj$Variable[i]=="Left.Knee"){
    df_Traj$Joint[i] = "Knee"
    df_Traj$Side[i] = "Left"
  }else if(df_Traj$Variable[i]=="Right.Knee"){
    df_Traj$Joint[i] = "Knee"
    df_Traj$Side[i] = "Right"
  }else if(df_Traj$Variable[i]=="Left.Hip"){
    df_Traj$Joint[i] = "Hip"
    df_Traj$Side[i] = "Left"
  }else{
    df_Traj$Joint[i] = "Hip"
...
df_Traj_FR <- df_Traj %>%
  filter(Cost==1, Simulation=="Simulation 1")
df_Traj_Naive <- df_Traj %>%
  filter(Cost==0, Simulation=="Simulation 1")
...
```{r}
ggplot(df_Traj_Naive,aes(x=Time, y=Joint_Angle, group=Speed, col=Speed)) +
#  geom_line(aes(x=Time, y=Limb_Angles, col=Speed), size=.9) +
  geom_smooth(show.legend = T) +
  facet_grid(Joint~Side) +
  geom_line(data=df_Long,aes(x=Time, y=Joint_Angle, group=Speed), col="black", size=.3, linetype=2, show.legend = T) +
  #scale_fill_manual(values = c("#0072B250","#D55E0050",  "red"), name= "Inferred from") +
  #scale_color_manual(values = c("#B55E00", "#1072B2", "red"), name="", labels= c("", "", "True Weights")) +
...
matfile = readMat('Trajectories_Bayesian_10Subjects_5Weights.mat')
# matfile = readMat('ResultsChains2500iter_20230222_BayesianIOC_Bases.mat')
...
Tibia.Stance = matfile$Tibia.Stance
Tibia.Swing = matfile$Tibia.Swing
Time.Demo = matfile$Time.Demo
Torso.Demo = matfile$Torso.Demo
Femur.Stance.Demo = matfile$Femur.Stance.Demo
Femur.Swing.Demo = matfile$Femur.Swing.Demo
Tibia.Stance.Demo = matfile$Tibia.Stance.Demo
Tibia.Swing.Demo = matfile$Tibia.Swing.Demo
Speed = matfile$Speed
Weight  = matfile$Weight
#Simulation = paste('Simulation', matfile$Simulation)
#Obs = matfile$Obs
RMSE=matfile$RMSE
Subject=matfile$Subject
Inference=matfile$Inference


df <- data.frame(Time, Torso=Torso, Femur.Swing = Femur.Swing, Femur.Stance=Femur.Stance, Tibia.Swing=Tibia.Swing, Tibia.Stance=Tibia.Stance, 
                 Speed=Speed, Weight=Weight, RMSE=RMSE, Subject=Subject, Inference=Inference)
df_Demo <- data.frame(Time.Demo, Torso.Demo=Torso.Demo, Femur.Swing.Demo = Femur.Swing.Demo, Femur.Stance.Demo=Femur.Stance.Demo, Tibia.Swing.Demo=Tibia.Swing.Demo, Tibia.Stance.Demo=Tibia.Stance.Demo,
                 Speed=Speed, Weight=Weight, RMSE=RMSE, Subject=Subject, Inference=Inference)

df_Traj <- df %>% pivot_longer(cols=2:6, names_to = "Variable", values_to="Limb_Angles") 

df_Traj_Demo <- df_Demo %>%
  pivot_longer(cols=2:6, names_to = "Variable_Demo", values_to="Limb_Angles_Demo")

View(df_Traj_Demo)

Subj = 1

df_Traj_S1_Naive <- df_Traj %>%
  filter(Subject==Subj, Inference==1)
df_Traj_S1_Naive_Demo <- df_Traj_Demo %>%
  filter(Subject==Subj, Inference==1)

df_Traj_S1_FR <- df_Traj %>%
  filter(Subject==Subj, Inference==2)
df_Traj_S1_FR_Demo <- df_Traj_Demo %>%
  filter(Subject==Subj, Inference==2)
...
df_Traj_S1_Naive_Summary <- df_Traj_S1_Naive %>%
  group_by(Subject, Variable, Inference, Speed, Weight, Time) %>%
  mutate(Mean_Angle=mean(Limb_Angles), sd_Angle=sd(Limb_Angles), Subject, Variable, Inference, Speed, Weight)
df_Traj_S1_Naive_Demo_Summary <- df_Traj_S1_Naive_Demo %>%
  group_by(Subject, Variable_Demo, Inference, Speed, Weight) %>%
  group_by(Time.Demo) %>%
  mutate(Mean_Angle=mean(Limb_Angles_Demo), sd_Angle=sd(Limb_Angles_Demo), Subject, Variable_Demo, Inference, Speed, Weight)

```


```{r}
ggplot(df_Traj_S1_Naive_Summary,aes(x=Time, y=Mean_Angle, group=Speed, col=Speed, fill=Speed)) +
#  geom_line(aes(x=Time, y=Limb_Angles, col=Speed), size=.9) +
  geom_smooth(show.legend = T) +
  geom_smooth(data=df_Traj_S1_Naive_Demo,aes(x=Time.Demo, y=Limb_Angles_Demo, group=Speed), col="red", size=.3, linetype=2, show.legend = T) +
  facet_grid(Variable~Weight) +
...
```{r}
ggplot(df_Traj_S1_Naive_Summary,aes(x=Time, y=Mean_Angle, group=Speed, col=Speed, fill=Speed)) +
  geom_ribbon(aes(ymin = Mean_Angle - sd_Angle, ymax = Mean_Angle + sd_Angle), fill = "grey70") +
  geom_line(aes(y = Mean_Angle)) +
  facet_grid(Variable~Weight) +
  scale_color_gradient(low = "skyblue", high = "darkblue")
```
...
```{r}
ggplot(df_Traj_S1_FR,aes(x=Time, y=Limb_Angles, group=Speed, col=Speed, fill=Speed)) +
#  geom_line(aes(x=Time, y=Limb_Angles, col=Speed), size=.9) +
  geom_smooth(show.legend = T) +
