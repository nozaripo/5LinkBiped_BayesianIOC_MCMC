function u = dynamics_forward(t,x,ddq,p)
...
if nargout == 1   % Using numerical gradients
    ...
    for i=1:nt
        M(mi) = m(:,i);
        F(fi) = f(:,i);
        u(:,i) = F-M*ddq(:,i);  %Numerically invert the mass matrix
    end

else  % Using analytic gradients
    [m,mi,f,fi,mz,mzi,mzd,fz,fzi,fzd] = autoGen_dynForward(...
        q(1,:),q(2,:),q(3,:),q(4,:),q(5,:),...
        dq(1,:),dq(2,:),dq(3,:),dq(4,:),dq(5,:),...
        u(1,:),u(2,:),u(3,:),u(4,:),u(5,:),...
        p.m1, p.m2, p.m3, p.m4, p.m5, p.I1, p.I2, p.I3, p.I4, p.I5, p.l1, p.l2, p.l3, p.l4, p.c1, p.c2, p.c3, p.c4, p.c5, p.g, empty);
